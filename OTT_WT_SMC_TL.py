# ==========================================
# MARKET SCANNER - ULTIMATE PRO MAX (V.ATR TP + OB RANGES + TRENDLINE BREAKS)
...
SPREADSHEET_ID = "16fT1IQxeDesoqYlNzw5yQpPHCgSgO6vfZgk4HvmvPWI"
...
TL_LENGTH      = 14       # Swing Detection Lookback
TL_MULT        = 1.0      # Slope multiplier
...
def calculate_trendlines(df, length=14, mult=1.0, calc_method='Atr'):
    """
    Translasi PineScript 'Trendlines with Breaks [LuxAlgo]' ke Python.
    Menggunakan mode non-backpaint (real-time) agar cocok untuk scanner.

    Output kolom yang ditambahkan ke df:
      - TL_Upper  : nilai upper trendline per bar
      - TL_Lower  : nilai lower trendline per bar
      - TL_UPos   : 1 jika harga sudah breakout ke atas upper trendline
      - TL_DNos   : 1 jika harga sudah breakout ke bawah lower trendline
      - TL_Breakout: sinyal breakout bar ini (1=up, -1=down, 0=none)
    """
    n      = len(df)
    ...
    # --- Pivot High / Low (ekuivalen ta.pivothigh / ta.pivotlow di Pine) ---
    # Pivot terkonfirmasi di bar ke-i jika bar tersebut adalah max/min
    # dalam window [i-length .. i+length]. Nilai dikembalikan di bar i+length
    # (delayed), sama seperti Pine yang mengembalikan nilai di bar saat ini
    # setelah length bar berlalu.
    ...
    for i in range(length, n - length):
        ...
        if highs[i] == np.max(window_high):
            ph_arr[i + length] = highs[i]   # konfirmasi di bar i+length
        if lows[i] == np.min(window_low):
            pl_arr[i + length] = lows[i]    # konfirmasi di bar i+length
    ...
    if calc_method == 'Atr':
        ...
        stdev_arr  = pd.Series(closes).rolling(window=length, min_periods=1).std().values
        slope_arr  = stdev_arr / length * mult
    else:  # Linreg
        slope_arr = np.zeros(n)
        idx_arr   = np.arange(n, dtype=float)
        for i in range(length, n):
            x = idx_arr[i - length: i]
            y = closes[i - length: i]
            if len(x) >= 2:
                cov_xy = np.mean(x * y) - np.mean(x) * np.mean(y)
                var_x  = np.var(x)
                slope_arr[i] = (abs(cov_xy) / var_x / 2 * mult) if var_x != 0 else 0

    # --- Tracking state (mirip var di Pine) ---
    upper      = 0.0
    lower      = 0.0
    slope_ph   = 0.0
    slope_pl   = 0.0
    upos       = 0       # 1 = sudah di atas upper (belum reset oleh pivot baru)
    dnos       = 0       # 1 = sudah di bawah lower

    tl_upper   = np.zeros(n)
    tl_lower   = np.zeros(n)
    tl_upos    = np.zeros(n)
    tl_dnos    = np.zeros(n)
    tl_breakout = np.zeros(n)   # +1 upward break, -1 downward break

    for i in range(n):
        sl = slope_arr[i]

        # Update slope saat pivot terkonfirmasi
        ...
        # Simpan nilai trendline (mode non-backpaint: upper - slope_ph * length)
        ...
        if not np.isnan(ph_arr[i]):
            upos = 0   # pivot baru reset posisi
        elif closes[i] > tl_upper[i]:
            upos = 1

        if not np.isnan(pl_arr[i]):
            dnos = 0   # pivot baru reset posisi
        ...
        # Deteksi breakout bar ini (upos naik dari 0→1, atau dnos naik dari 0→1)
        if upos > prev_upos:
            tl_breakout[i] = 1    # upward break
        elif dnos > prev_dnos:
            tl_breakout[i] = -1   # downward break
...
def analyze_sector(sector_name, ticker_list):
    ...
    for ticker in ticker_list:
        # --- PERTAHANAN LAPIS 1: Inisialisasi Default ---
        # Ini mencegah UnboundLocalError secara absolut
        price_today = 0.0
        var_today = 0.0
        ott_today = 0.0
        target_tp = 0.0
        ...
        try:
            ...
            # --- PERTAHANAN LAPIS 2: Ekstraksi YFinance v0.2.40+ ---
            # Mengatasi MultiIndex dan mengubah kolom jadi huruf kapital (Close, High, Low)
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = [str(c[0]).capitalize() for c in df.columns]
            else:
                df.columns = [str(c).capitalize() for c in df.columns]
                
            # --- PERTAHANAN LAPIS 3: Validasi Keamanan Data ---
            if df.empty or len(df) < 100 or 'Close' not in df.columns or 'High' not in df.columns:
                continue
            ...
            # ============================================
            # EKSTRAKSI HARGA & INDIKATOR — Aman karena Lapis 1 & 3
            # ============================================
...
if __name__ == "__main__":
    ...
    print("   5. Trendlines with Breaks (LuxAlgo)")
    print("=" * 65)
