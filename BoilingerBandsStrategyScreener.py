"""
IHSG Bollinger Bands Screener v6.0 (Complete Trading System - Trial)
=============================================================
...
# ── CONFIG ─────────────────────────────────────────────────────────────────
SPREADSHEET_ID  = "1HESgWfROm5dVqymC_WwY3gTeQySX93wfT-_XYCWBe-Q"
...
DISPLAY_COLS = [
    ...
    "Bollinger Bands Signal", "Tgl BB Signal", "Skor Tambahan", "ADTV (M)",
...
def calc_bb(df, length=BB_LENGTH, mult=BB_MULT):
    """Menghitung Bollinger Bands Strategy"""
    # TradingView secara default menggunakan standar deviasi populasi (ddof=0)
    ...
    # Strategy: 
    # Long ketika Close ditutup di bawah lower band (Oversold -> Mean Reversion)
    # Short ketika Close ditutup di atas upper band (Overbought -> Mean Reversion)
    le_condition = df["Close"] < lower
    se_condition = df["Close"] > upper
    ...
    # SL for BB Mean Reversion: Jika Long, SL adalah Lower Band-nya
    df_sig['sl_price'] = np.where(df_sig['state'] == 1, lower, upper)
    ...
    if lb <= sb:
        return {"label": fmt_label(lb, "My Long Entry Id"), "date": fmt_date(le_dt), "type": "BB Long", "bars": lb, "sl": curr_sl}
    else:
        return {"label": fmt_label(sb, "My Short Entry Id"), "date": fmt_date(se_dt), "type": "BB Short", "bars": sb, "sl": curr_sl}
...
def calc_custom_score(df, sector, current_price):
    ...
    # 4. Kategori Strategi Mean Reversion (+1 Poin)
    if strat_cat == "Mean Reversion":
        score += 1
        reasons.append("Cocok Bollinger Bands")
...
def analyze(ticker, sector, ctx):
    df = get_ohlcv(ticker)
    if df is None or len(df) < max(BB_LENGTH, 52)+2:
    ...
    return {
        ...
        "Action"                  : action,
        "Harga"                   : int(close_now),
        "Batas Jual (SL)"         : int(bb["sl"]),
        "ADTV (M)"                : adtv,
        "Tgl Data"                : tgl,
        "Bollinger Bands Signal"  : bb["label"],
        "Tgl BB Signal"           : bb["date"],
...
def run_sector(sector, tickers, ctx):
    ...
    for i,t in enumerate(tickers,1):
        ...
        if r: rows.append(r); print(r["Bollinger Bands Signal"])
        else: print("skip")
        time.sleep(0.3)
    if not rows: return pd.DataFrame()
    df=pd.DataFrame(rows)
    df["_le"]=(df["_type"]=="BB Long").astype(int)
...
def upload_sector(sector, df, ctx):
    ...
    try:
        ...
        ws.update("A1",[[f"📊 {sector} | Bollinger Bands (Length={BB_LENGTH}, Mult={BB_MULT}) | {ts}"]])
...
def upload_summary(all_rows, ctx):
    ...
    try:
        ...
        ts=datetime.now(WIB).strftime("%Y-%m-%d %H:%M WIB")
        ws.update("A1",[[
            f"🔍 IHSG Bollinger Bands Screener v6.0 — {ts}","",
            f"BB Long = Oversold (Buy) | BB Short = Overbought (Sell) | auto_adjust=False"]])
        ...
        df["_le"]=(df.get("_type","")=="BB Long").astype(int)
...
def main():
    ts=datetime.now(WIB).strftime("%Y-%m-%d %H:%M WIB")
    print(f"\n{'═'*60}\n  IHSG Bollinger Bands Screener v6.0 (Trial Mode)    {ts}")
    print(f"  Length={BB_LENGTH} | Mult={BB_MULT} | {DOWNLOAD_DAYS}d history")
    print(f"{'═'*60}\n")
    ...
    print(f"  BB Long         : {df_f['Bollinger Bands Signal'].str.contains('My Long Entry Id',na=False).sum()}")
    print(f"  BB Short        : {df_f['Bollinger Bands Signal'].str.contains('My Short Entry Id',na=False).sum()}")
    
    print(f"  Signal HARI INI : {df_f['Bollinger Bands Signal'].str.contains('hari ini',na=False).sum()}")
    print(f"{'═'*60}\n")
